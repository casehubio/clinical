...
**Test ledger repository — use `InMemoryLedgerEntryRepository`, NOT `JpaLedgerEntryRepository`:** A `casehub-ledger` snapshot introduced `LedgerSequenceAllocator` (native SQL `MERGE INTO ledger_subject_sequence`). This table is not Hibernate-mapped and is not created by `drop-and-create` in tests. `JpaLedgerEntryRepository` fails with "Table LEDGER_SUBJECT_SEQUENCE not found" on any ledger write. Use `casehub-ledger-memory:InMemoryLedgerEntryRepository` in test `selected-alternatives`. Requires `casehub-ledger-memory` as `<scope>test</scope>` dependency and `quarkus.index-dependency.ledger-memory` entry. See `SponsorNotificationStoreTest` and `DeviationExpirerTest` for why this matters.
