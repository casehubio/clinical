...
**Status:** Layers 1–6 complete. Layer 7 stub. All Chapters complete.
...
Current delivery state — Layers 1–6 complete. Layer 7 pending.
...
Missing connector config (safety officer notification, sponsor notification): logs a warning; writes no ledger entry. Tracked as #49 — early-return audit gap. See Anti-patterns below.
...
- **Early-return paths leave no ledger trace**
  ...
  - **Fix:** Write a ledger entry for every early-return path — `delivered=false`, `connectorId=null` (distinguishes "error before notifier reached" from "notifier ran but delivery failed"). Tracked: casehubio/clinical#49.
...
- `runtime/src/main/resources/ae-escalation.yaml` — AE case definition: Grade 3 → senior monitor gate; Grade 4+ → + DSMB gate in parallel
- `runtime/src/main/resources/deviation-review.yaml` — deviation case definition: IRB gate with 72h deadline; 4 terminal outcomes
...
9. Test binding conditions with unit tests using `MapCaseContext` (no Quarkus required); write a `@QuarkusTest` asserting YAML loads cleanly with expected binding/goal counts
...
### Layer — Trust Routing (stub)

**Participates in chapters:** *(pending — C6 when implemented)*
**Completed:** 🔲 Not yet built — casehubio/clinical#10

🔲 Full entry at Layer 7 close. Blocked on Layer 7 implementation.

Known scope: `TrustWeightedSelectionStrategy` for clinical capability routing; `ActorTrustScore` updated from WorkItem outcomes and commitment attestations (Bayesian Beta); routing selects agent with highest domain-specific trust score — `safety-accuracy` for AE classification, `eligibility-precision` for screening, `protocol-adherence` for deviation review.

**Regulatory gap (21 CFR Part 312.32):** `DefaultAdverseEventEscalationPolicy` currently routes Grade 5 identically to Grade 4 (senior monitor + DSMB). Under 21 CFR 312.32(c)(1)(i), the expedited IND safety reporting obligation applies to *unexpected* fatal events — a listed expected outcome uses a different reporting window. Layer 7 must add `boolean unexpected` to `AeEscalationCompletedEvent` and a `regulatory-submission` binding that fires when `grade == GRADE_5 && unexpected`. Tracked: casehubio/clinical#30.
...
|---|---|---|
| engine API incompatibility — `startCase(CaseDefinition, Object)` missing from `CaseHubRuntimeImpl`; 12 lifecycle tests failing | Blocked on engine-side fix; no clinical code change required | casehubio/clinical#53 |
| Early-return audit gap — missing-config paths in notification listeners leave no ledger trace | `delivered=false` entries cover connector failures; silent skips (site not found, config missing) still leave no trace | casehubio/clinical#49 |
| Grade 5 regulatory gap — `DefaultAdverseEventEscalationPolicy` does not evaluate `unexpected` qualifier (21 CFR 312.32) | Documented limitation; Layer 7 must add `boolean unexpected` to `AeEscalationCompletedEvent`; see Layer 7 stub in §9.4 | casehubio/clinical#10 (Layer 7 scope) |
| Layer 7 trust routing absent — agents selected by availability | Bootstrap mode; routing quality improves as attestations accumulate | casehubio/clinical#10 |

### Technical Debt

| Item | Description | Layer | Tracked |
|---|---|---|---|
| Test coverage gaps | `enrollmentId` null path in `AeEscalationListener`; `IrbDecisionListenerTest` missing `approvalId` assertion | L5 | casehubio/clinical#52 |
| `actorId` inconsistency | Some ledger writers use `"system"` rather than `ClinicalActors.CLINICAL_SERVICE` constant — pre-existing, partially fixed | L4, L5 | casehubio/clinical#46 |
| `StubWorkloadProvider` in every test module | `JpaWorkloadProvider` excluded; `StubWorkloadProvider` (`@DefaultBean`, returns 0) must be added to any new test module adding engine | L5 | casehubio/clinical#41 |
...
**Layer 5 — yes, with one gap.** The 9-step Pattern covers `ledgerWritten` flag, three-phase activation, Quartz incompatibility, and `GoalReached` vs `CaseCompleted`. The gap: no YAML snippet showing the actual binding condition structure (grade evaluation, parallel humanTask). A reader replicating Layer 5 must look at the YAML files directly to understand binding syntax.
...
- A YAML snippet in §9.4 Layer — casehub-engine showing the core binding structure (Grade 3 vs Grade 4+ conditional, `humanTask` parallel firing). Would make Layer 5 replication fully self-contained.
- `SafetyOfficerNotificationLedgerEntry` (V1011) key file entry in Layer 3 extension section — currently only in LAYER-LOG.md clinical#11 note.
