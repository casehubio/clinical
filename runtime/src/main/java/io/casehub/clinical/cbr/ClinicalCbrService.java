...
public class ClinicalCbrService {
    ...
    @Inject
    public ClinicalCbrService(final CbrRecordStore store,
    ...
    public String storeIdempotent(final CbrRecord cbrCase, final String caseType,
    ...
    public <C extends CbrRecord> List<CbrMatch<C>> retrieveSimilar(final CbrQuery query,
                                                                      final Class<C> caseType) {
        return store.retrieveSimilar(query, caseType);
    }

    public <C extends CbrRecord> AuditedRetrievalResult<C> retrieveWithAudit(
            final CbrQuery query, final Class<C> caseType,
            final java.util.UUID subjectId, final String actorId) {
        List<CbrMatch<C>>                            cases = retrieveSimilar(query, caseType);
    ...
    private <C extends CbrRecord> io.casehub.neocortex.memory.cbr.CbrRetrievalTrace buildTrace(
            CbrQuery query, List<CbrMatch<C>> cases) {
        List<io.casehub.neocortex.memory.cbr.CbrRetrievalTrace.TracedCase> tracedCases = cases.stream()
                                                                                              .map(sc -> new io.casehub.neocortex.memory.cbr.CbrRetrievalTrace.TracedCase(sc.caseId(), sc.score(), sc.reranked(), sc.featureSimilarities(), sc.cbrRecord().confidence(), null, null, null))
                                                                                              .toList();
