...
class SiteEnrollmentAlertServiceTest {
    ...
    void noMatches_returnsEmpty() {
        ...
        when(cbrService.retrieveWithAudit(any(), eq(CbrFeatureRecord.class), any(), any()))
    ...
    private CbrMatch<CbrFeatureRecord> scoredCase(String outcome, double score) {
        var cbrCase = new CbrFeatureRecord("problem", "solution", outcome, Confidence.unknown(1.0), Map.of(), null, null);
        return new CbrMatch<>(cbrCase, UUID.randomUUID().toString(), "clinical-ae", score);
    }
}
